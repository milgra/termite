...
void renderer_init(GLuint defaultFrameBuffer, GLuint defaultRenderBuffer, char* resources);
...
void renderer_onmessage(const char* name, void* data);
...
void renderer_create_framebuffers()
{
    ...
    ogl_framebuffer_with_texture(&rnd.fb_game, &rnd.tex_game, &rnd.depth_game, fulld, fulld);

    ogl_framebuffer_with_texture(&rnd.fb_glow, &rnd.tex_glow, &rnd.depth_glow, glowd, glowd);

    ogl_framebuffer_with_texture(&rnd.fb_blur, &rnd.tex_blur, &rnd.depth_blur, blurd, blurd);
}
...
void renderer_init(GLuint defFrameBuffer, GLuint defRenderBuffer, char* resources)
{
    ...
    rnd.shader_glow = ogl_shader_create(glow_vsh, glow_fsh, uniforms_game, attributes_game, rnd.gameuniforms);

    rnd.shader_blur = ogl_shader_create(blur_vsh, blur_fsh, uniforms_blur, attributes_blur, rnd.bluruniforms);

    rnd.shader_blend = ogl_shader_create(blend_vsh, blend_fsh, uniforms_blend, attributes_blend, rnd.blenduniforms);

    ogl_vertexbuffer(&rnd.vbo_main);

    renderer_create_framebuffers();

    glBindRenderbuffer(GL_RENDERBUFFER, rnd.defaultRenderBuffer);
}

void renderer_free()
{
    REL(rnd.dynamicbuffer);
}
...
void renderer_update_matrix(m4_t projection, int level)
{
    rnd.projections[level] = projection;
}
...
void renderer_draw_unit_quad()
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, rnd.vbo_main);
...
void renderer_render_scene(int* levelmap, int* drawmap)
{
	...
	glUniformMatrix4fv(rnd.gameuniforms[0], 1, 0, projection.array);
	...
	if (rnd.vbocounts[level] > 0)
	{
	    ...
	    glDrawArrays(GL_TRIANGLE_STRIP, 0, rnd.vbocounts[level] / 4);
	}
    }
}
...
void renderer_render()
{
    ...
    int levelmap[]      = {4, 0, 1, 2, 3};
    int drawmap[]       = {4, 1, 1, 1, 1};
    int levelmap_glow[] = {3, 0, 1, 2};
    int drawmap_glow[]  = {4, 0, 1, 1};
	...
	glBindFramebuffer(GL_FRAMEBUFFER, rnd.fb_game);

	glViewport(0, 0, fulld, fulld);
    }
    else
    {

	glBindFramebuffer(GL_FRAMEBUFFER, rnd.defaultFrameBuffer);
	glBindRenderbuffer(GL_RENDERBUFFER, rnd.defaultRenderBuffer);

	glViewport(0.0, 0.0, rnd.width, rnd.height);
    }

    glClearColor(defaults.backcolor.x, defaults.backcolor.y, defaults.backcolor.z, 1.0);

    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

    glUseProgram(rnd.shader_glow);

    renderer_render_scene(levelmap, drawmap);
	...
	glBindFramebuffer(GL_FRAMEBUFFER, rnd.fb_glow);

	glClearColor(0.0, 0.0, 0.0, 1.0);

	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

	glViewport(0.0, 0.0, glowd, glowd);

	renderer_render_scene(levelmap_glow, drawmap_glow);

	// render contents of glow framebuffer with horizontal blur shader into blur framebuffer

	glBindFramebuffer(GL_FRAMEBUFFER, rnd.fb_blur);
	...
	glViewport(0.0, 0.0, blurd, blurd);
	...
	glUniform1i(rnd.bluruniforms[1], 1); // draw vertical direction
...
void renderer_uploadvoxels(void* data)
{
	...
	voxel_update(voxel);
	voxel_collect_points(voxel, rnd.dynamicbuffer);
    }

    renderer_update_buffer(rnd.dynamicbuffer->data, rnd.dynamicbuffer->length, rdata->projection, rdata->level);
}
...
void renderer_onmessage(const char* name, void* data)
{
    if (strcmp(name, "UPLOADVOXELS") == 0) renderer_uploadvoxels(data);
    else if (strcmp(name, "RENDER") == 0) renderer_render();
    else if (strcmp(name, "RESIZE") == 0) renderer_resize(data);
    else if (strcmp(name, "EFFECTS_LEVEL") == 0) renderer_set_detail();
}
